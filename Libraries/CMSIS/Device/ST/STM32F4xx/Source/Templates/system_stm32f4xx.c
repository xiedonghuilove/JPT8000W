...
void SystemInit(void)
{
  ...
  #endif
  /* Reset the RCC clock configuration to the default reset state ½«RCCÊ±ÖÓÅäÖÃÄ¬ÈÏµÄÖØÖÃ×´Ì¬ ------------*/
  /* Set HSION bit  ÄÚ²¿¸ßËÙÊ±ÖÓÊ¹ÄÜ*/
  RCC->CR |= (uint32_t)0x00000001;

  /* Reset CFGR register ¸´Î»Ê±ÖÓÅäÖÃ¼Ä´æÆ÷ */
  RCC->CFGR = 0x00000000;

  /* Reset HSEON, CSSON and PLLON bits ¸´Î»HSEON£¬CSSONºÍPLLONÎ» 
		HSEON --> bit16 = 0 HSE Õñµ´Æ÷¹Ø±Õ
		CSSON --> bit19 =0 Ê±ÖÓ°²È«ÏµÍ³Ê¹ÄÜÅäÖÃ 
		PLLON --> bit24 =0 Ö÷PLLÊ¹ÄÜÅäÖÃ
	*/
  RCC->CR &= (uint32_t)0xFEF6FFFF;

  /* Reset PLLCFGR register ¸´Î»PLLCFGR¼Ä´æÆ÷ PLLÅäÖÃ¼Ä´æÆ÷*/
  RCC->PLLCFGR = 0x24003010;

  /* Reset HSEBYP bit18 = 0  HSEÊ±ÖÓÅÔÂ· */
  RCC->CR &= (uint32_t)0xFFFBFFFF;

  /* Disable all interrupts Ê§ÄÜËùÓÐÖÐ¶Ï*/
  RCC->CIR = 0x00000000;
  ...
  /* Configure the System clock source, PLL Multiplier and Divider factors, 
	 ...
	 ÅäÖÃÏµÍ³Ê±ÖÓÔ´£¬PLL±¶ÆµÆ÷ºÍ·ÖÆµÆ÷Òò×Ó£¬
	 AHB / APBxÔ¤·ÖÆµÆ÷ºÍÉÁ¹âµÆÉèÖÃ----------------------------------*/
  SetSysClock();
...
#if defined(STM32F40_41xxx) || defined(STM32F427_437xx) || defined(STM32F429_439xx) || defined(STM32F401xx) || defined(STM32F446xx)
/******************************************************************************/
/*            PLL (clocked by HSE) used as System clock source   PLL£¨ÓÉHSEÌá¹©Ê±ÖÓ£©ÓÃ×÷ÏµÍ³Ê±ÖÓÔ´              */
/******************************************************************************/
  __IO uint32_t StartUpCounter = 0, HSEStatus = 0;
  
  /* Enable HSE Ê¹ÄÜHSEÊ±ÖÓ*/
  RCC->CR |= ((uint32_t)RCC_CR_HSEON);
 
  /* Wait till HSE is ready and if Time out is reached exit µÈµ½HSE×¼±¸¾ÍÐ÷£¬Èç¹û´ïµ½³¬Ê±ÔòÍË³ö */
  do
  {
    HSEStatus = RCC->CR & RCC_CR_HSERDY;//Ê±ÖÓ¾ÍÐ÷±êÖ¾ = 1
    StartUpCounter++;
