...
void ee_ReadTest(void)
{
	...
	else
	{
		...
		printf("Read eeprom Succuss,Data list:\r\n");
	}
	...
	/* 打印读速度 */
	printf("Reading time: %dms, Reading speed: %dB/s\r\n", iTime2 - iTime1, (EE_SIZE * 1000) / (iTime2 - iTime1));
}
...
void ee_WriteTest(void)
{
	...
	{
		iTime2 = bsp_GetRunTime();	/* 记下结束时间 */
		printf("Write eeprom Succuss\r\n");
	}
	/* 打印读速度 */
	printf("Write time: %dms, Write Speed: %dB/s\r\n", iTime2 - iTime1, (EE_SIZE * 1000) / (iTime2 - iTime1));
}
